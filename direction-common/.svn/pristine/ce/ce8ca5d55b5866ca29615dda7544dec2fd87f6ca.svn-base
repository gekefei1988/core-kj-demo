package com.direction.common.utils.codec;
...
import com.direction.common.utils.lang.ExceptionUtils;
...
public class EncodeUtils {
	...
	private static List<Pattern> getXssPatterns() {
		if (xssPatterns != null) {
			return xssPatterns;
		}
		...
		xssPatterns.add(Pattern.compile("\\s*on[a-z]+\\s*=\\s*(\"[^\"]+\"|'[^']+'|[^\\s]+)\\s*(?=>)", Pattern.CASE_INSENSITIVE));
