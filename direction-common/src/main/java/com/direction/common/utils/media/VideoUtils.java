package com.direction.common.utils.media;
...
import com.direction.common.utils.io.FileUtils;
import com.direction.common.utils.io.PropertiesUtils;
import com.direction.common.utils.lang.ObjectUtils;
import com.direction.common.utils.lang.TimeUtils;
...
public class VideoUtils {
	...
	private int checkContentType() {
		...
		if (ArrayUtils.contains(new String[] {
		  ...
		  "rm",
		  "rmvb"
		}, inputFileExtension.toLowerCase())) {
			return 0;
		}
		// 对ffmpeg无法解析的文件格式(wmv9，等), 可以先用别的工具（mencoder）转换为avi(ffmpeg能解析的)格式.
		else if (StringUtils.equalsIgnoreCase(inputFileExtension.toLowerCase(), "wmv9")) {
			return 1;
		}
	...
	public boolean cutPic() {
		...
		if (statusTemp) {
			...
			try {
				...
				if (imgfile.exists()) {
					...
					if (!StringUtils.equalsIgnoreCase(FileUtils.getFileExtension(imgFile), "gif")) {
						Integer w = 800;
						Integer h = 600;
		...
		log.debug("视频剪切图片"
		    + (statusTemp ? "成功" : "失败")
		    + "，用时："
		    + TimeUtils.formatDateAgo(System.currentTimeMillis() - startTime));
		return statusTemp;
	}
	...
	public boolean convert() {
		...
		log.debug(
		    "视频转换" + (statusTemp ? "成功" : "失败") + "，用时：" + TimeUtils.formatDateAgo(System.currentTimeMillis() - startTime));
		return statusTemp;
	}
